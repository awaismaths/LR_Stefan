...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%clear all;
function [ITER_SP,TSP,Res,L2u, L2z, H1z]=main_comp_sp(tol,nvert,cell_n,diam,ncell,vertex,area,cell_v,dt_initial,mesh)
imesh=1;
meshes{imesh}=mesh;
...
% %     % compute error
      %[Lp1_error(idt) L2zeta_error(idt) H1zeta_error(idt)] = compute_errors(usol,ex_sol,dualarea,Diffmat,I_indices,zcase,ucase);
...
%     H1z=(dt*sum( H1zeta_error.^2))^(1/2);
 %%%%%%%%%%%%%%%%%%%%%%%%%%  
   ex_sol=exact_solution(dt*idt,vertex,ucase)';
[L2u L2z H1z] = compute_errors(usol,ex_sol,dualarea,Diffmat,I_indices,zcase,ucase);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5 
%str = sprintf('Newton: Mesh %i, num_time_steps=%d, Avg_iter=%d with total_relax=%d, Avg_residue=%4.2e in %f seconds\n',imesh,Ndt(imesh),ITER,num_updates,Res,Time);
    % forkprint(fid,str);
