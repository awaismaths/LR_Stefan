function [ITER_LSSP,TLSSP,Res_LSSP]=main_comp_lssp(tol,Lc,nbm,nvert,cell_n,diam,ncell,vertex,area,cell_v,dt,mesh)
format long;
imesh=1;
meshes{imesh}=mesh;
format long;
...
%     Sol=load(strcat('solutions/LSSP_BM',num2str(bmm),'mesh',meshes{imesh}(5:7),'tcuz',num2str(ucase),num2str(zcase)));
%     Sol=Sol.usol_idt;
%     % Exact solution
...
Time=toc(tstartL);
ITER_LSSP=mean(ITER_LSSP);
Res_LSSP=mean(Res_LSSP);
TLSSP=Time;
