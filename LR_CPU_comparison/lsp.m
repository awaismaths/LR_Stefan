...
%clear all;
function [ITER_LSP,TLSP,Res,L2u, L2z, H1z]=main_comp_lsp(tol,Lc,nvert,cell_n,diam,ncell,vertex,area,cell_v,dt,mesh)
format long;
imesh=1;
meshes{imesh}=mesh;
...
%meshes={'mesh1_01.mat';'mesh1_02.mat';'mesh1_03.mat';'mesh1_04.mat';'mesh1_05.mat';'mesh1_06.mat'};%'mesh1_7.mat'};
...
%           Xsol(I_indices)=G(I_indices,I_indices)\F(I_indices);
        ...
        %[Lp1_error(idt) L2zeta_error(idt) H1zeta_error(idt)] = compute_errors(usol,ex_sol,dualarea,A,I_indices,zcase,ucase);
...
%%%%%%%%%%%%%%%%%%
ex_sol=exact_solution(dt*idt,vertex,ucase)';
       [L2u L2z H1z] = compute_errors(usol,ex_sol,dualarea,A,I_indices,zcase,ucase);
%%%%%%%%%%%%%%%%%%%%%%%%
%str = sprintf('Linearised: Mesh %i, num_time_steps=%d, Avg_iter=%d with Avg_residue=%4.2e in %f seconds\n',imesh,Ndt(imesh),ITER,Res,Time);
     %forkprint(fid,str);
%     clear ITER Res
%            % Exact solution
%       ex_sol=exact_solution(dt*idt,vertex,ucase)';
% %      % compute error
%      [Lp1_error L2zeta_error H1zeta_error] = compute_errors(usol,ex_sol,dualarea,A,I_indices,zcase,ucase);
% % 
