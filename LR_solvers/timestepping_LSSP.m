function [ITER,Res]=timestepping_LSSP(tol,Lc,zcase,ucase,bmm,dW,ncell,nvert,area,cg,cell_v,X,Mass,L,A,G,B_indices,I_indices,imesh,Ndt,dt,bdry_vert,mesh,h)
%global ucase zcase
%tol=1e-8;
itermax=5000;
...
for idt=1:Ndt;
    ...
    %ub(B_indices)=(exact_solution(idt*dt,bdry_vert(B_indices,:),ucase))';
    rhs = Mass*Xprev + dt*b;
    iter = 0;
    res = 1;
    while (iter < itermax && res > tol)
        ...
        %X_b(B_indices)=ub(B_indices);%(1/Lc)*(zb(B_indices)-zetau(Xprev(B_indices),zcase))+Xprev(B_indices);
        F=B-G*X_b;
    ...
    if (iter==itermax)
     ...
     usol = X;
    usol(B_indices)=zetau(X(B_indices),zcase);
     usol_idt(:,idt+1)=usol;
    ITER=ITER+iter;
    Res=Res+abs(res);
...
save(strcat('solutions/LBM',num2str(bmm),'mesh',mesh(1:8),'tcuz',num2str(ucase),num2str(zcase)),'usol_idt','dt','Ndt','mesh','h','-v7.3');
end
