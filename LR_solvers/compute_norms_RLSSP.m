...
%Computing for each Brownian motion       
    for bmm=1:nbm
        ...
        CS=load(strcat('solutions/RBM',num2str(bmm),'mesh',meshes{imesh}(1:8),'tcuz',num2str(ucase),num2str(zcase)));
        FS=load(strcat('solutions/RBM',num2str(bmm),'mesh',meshes{nbmeshes}(1:8),'tcuz',num2str(ucase),num2str(zcase)));
...
h=[C_h;F_h];
filenameE=strcat('norms/T1RME',meshes{nbmeshes}(8:8),'bm',num2str(nbm),'.dat');
filenameV=strcat('norms/T1RMV',meshes{nbmeshes}(8:8),'bm',num2str(nbm),'.dat');
 % Format the output data
 
 output_titleV={'h' 'VLZ' 'VHZ' 'VXI'};
 output_dataV=[h L2zeta H1zeta L1Xi];
 outputV=[output_titleV;num2cell(output_dataV)];
 writecell(outputV,filenameV,'Delimiter','tab');
output_titleE={'h' 'ELZ' 'EHZ' 'EXI'};
output_dataE=[C_h Est_eznorm GEst_eznorm Est_eXi];
 outputE=[output_titleE;num2cell(output_dataE)];
 writecell(outputE,filenameE,'Delimiter','tab');