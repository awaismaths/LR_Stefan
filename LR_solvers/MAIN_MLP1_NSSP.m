%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Mass-lumped P1 codes with different solvers (Newton)
...
%          WF(:,bmm)=sqrt(dt)*dW;
%      end
%      save(strcat('BM/BM_mesh',num2str(meshes{nbmeshes}(1:8))),'WF');
%      clear WF;
%      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     %
...
%         end;
    ...
    %     write_solution_vtk(ex_sol,'V_0TKout/ex_sol0',ncell,nedge,nvert,cell_v,cell_n,cell_e,vertex);
 ...
 %% Loading brownian motion
    ...
    parfor bmm=1:nbm
        %plot(W(:,bmm));
        normvec=timestepping_NSSP(tol,zcase,ucase,bmm,W(:,bmm),ncell,nvert,vertex,area,cg,cell_v,X,Mass,A,B_indices,I_indices,imesh,Ndt(imesh),dt,bdry_vert,meshes{imesh},h,dualarea);
%     % Following error only works in the case of deterministic problem. Also, uncomment the correct source in the "timestepping". 
%     Sol=load(strcat('solutions/BM',num2str(bmm),'mesh',meshes{imesh}(5:7),'tcuz',num2str(ucase),num2str(zcase)));
%     Sol=Sol.usol_idt;
%     % Exact solution
...
% aama(imesh)=mean(ama)
%filename=strcat('norms/M',meshes{imesh}(8:8),'bm',num2str(nbm),'.dat');
%fid = fopen('filename', 'w');
%fprintf(fid,'normvec');
%writematrix(normvec,filename,'Delimiter','tab');
end; % end meshes
% aama
